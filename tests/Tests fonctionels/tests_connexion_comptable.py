...
#connexion du comptable
try:
    driver.find_element(By.ID,"comptable").click()
...
try:
    sql = """SELECT comptable.code FROM comptable WHERE comptable.login = %s"""
...
# Saisie du code A2F
try:
    setValeurInput('input[name="codemail"]', code)
    driver.find_element(By.TAG_NAME, 'form').submit()
